...
public class SequenceAligner {
        ...
        for (int i = 0; i <= n; i++) {
            for (int j = 0; j <= m; j++) {
                if (i == 0 && j == 0) {
                    ...
                    cache[i][j] = new Result(j * judge.getGapCost(), Direction.LEFT);
                } else if (j == 0) {
                    cache[i][j] = new Result(i * judge.getGapCost(), Direction.UP);
                } else {
                    ...
                    Direction dir = maxScore == diag ? Direction.DIAGONAL : maxScore == left ? Direction.LEFT : Direction.UP;
    ...
    private void traceback() {
        // Initialize cache[0][0] before starting the traceback process
        ...
        while (i > 0 || j > 0) {
            ...
            if (current.getParent() == Direction.DIAGONAL) {
                ...
                sbX.insert(0, Constants.GAP_CHAR);
                sbY.insert(0, y.charAt(j - 1));
                j--;
            } else if (current.getParent() == Direction.UP) {
                sbX.insert(0, x.charAt(i - 1));
                sbY.insert(0, Constants.GAP_CHAR);
                i--;
            }
        }
